 ...
 * @author Zhifan Yang
 */
class BoardAnimationReversed extends Sprite
{
  ...
  function initializeTilesheet():Void
  {
    ...
    var boardBitmapData:BitmapData = Assets.getBitmapData( "assets/rotationReversedTilesheet.png" );
    boardTilesheet = new Tilesheet( boardBitmapData );

    // loop to get all the seperate images from the sprite sheet
    for( rotationReversedTilesheetIndex in 0...boardCount )
    {
      ...
      row = Math.floor( rotationReversedTilesheetIndex / 4 );
      column = rotationReversedTilesheetIndex % 4;
    }
  }
}